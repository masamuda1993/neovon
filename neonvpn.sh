...
run_installer() {
    ...
    # Skip jika xray binary sudah ada (misal dari upload manual sebelumnya)
    if [[ -x "$XRAY_BIN" ]]; then
        _existing_ver=$("$XRAY_BIN" version 2>/dev/null | head -1)
        log_ok "Xray sudah terinstall: ${BWHT}${_existing_ver}${RST}"
        log_info "Skip download. Hapus $XRAY_BIN jika mau reinstall."
    else
    # --- Xray belum ada, mulai download/compile ---
    ...
    # --- Get latest xray version tag ---
    XRAY_VER=""
    log_info "Mendapatkan versi Xray terbaru ..."
    
    # Try multiple version-check sources (non-GitHub first)
    _ver_sources=(
        "https://cdn.jsdelivr.net/gh/XTLS/Xray-core@latest/release/config.json"
        "https://api.github.com/repos/XTLS/Xray-core/releases/latest"
        "https://ghfast.top/https://api.github.com/repos/XTLS/Xray-core/releases/latest"
    )
    for _vs in "${_ver_sources[@]}"; do
        _ver_raw=$(curl -fsSL --max-time 15 "$_vs" 2>/dev/null)
        if [[ -n "$_ver_raw" ]]; then
            # jsDelivr returns config.json; GitHub API returns JSON with tag_name
            XRAY_VER=$(echo "$_ver_raw" | grep -oP '"tag_name"\s*:\s*"v?\K[^"]+')
            [[ -z "$XRAY_VER" ]] && XRAY_VER=$(echo "$_ver_raw" | grep -oP '"version"\s*:\s*"\K[^"]+')
            [[ -n "$XRAY_VER" ]] && break
        fi
    done
    if [[ -z "$XRAY_VER" ]]; then
    ...
    # ── Mirror list: NON-GitHub CDNs first, then GitHub mirrors ──
    XRAY_MIRRORS=(
        # jsDelivr CDN — global CDN, NOT a GitHub proxy (highest success rate)
        "https://cdn.jsdelivr.net/gh/XTLS/Xray-core@v${XRAY_VER}/${XRAY_FILENAME}"
        # GitHub direct + mirrors (may be blocked on some VPS)
    ...
    for mirror_url in "${XRAY_MIRRORS[@]}"; do
        ...
        if curl -fSL --max-time 180 "$mirror_url" -o /tmp/neonvpn_xray.zip 2>/dev/null; then
            _stop_spinner
            # Verify it's a valid zip (min 5MB — xray binary is ~20MB+)
            _fsize=$(stat -c%s /tmp/neonvpn_xray.zip 2>/dev/null || echo 0)
            if [[ "$_fsize" -lt 5000000 ]]; then
                _stop_spinner 2>/dev/null
                log_warn "File terlalu kecil (${_fsize} bytes), coba mirror lain..."
    ...
    # ── FALLBACK: Compile Xray from source using Go ──
    if [[ "$xray_downloaded" != "true" ]]; then
        echo ""
        log_warn "Semua mirror download gagal. Mencoba compile dari source..."
        
        # Check if Go is installed
        if command -v go &>/dev/null; then
            log_info "Go sudah terinstall: $(go version)"
        else
            log_info "Menginstall Go compiler..."
            # Try multiple Go download sources
            _go_installed=false
            _go_ver="1.22.5"
            _go_file="go${_go_ver}.linux-${XRAY_ARCH}.tar.gz"
            _go_mirrors=(
                "https://dl.google.com/go/${_go_file}"
                "https://cdn.jsdelivr.net/gh/golang/go@go${_go_ver}/src/"
                "https://golang.google.cn/dl/${_go_file}"
            )
            # Try apt first (easiest)
            if apt-get install -y golang-go >/dev/null 2>&1; then
                log_ok "Go terinstall via apt"
                _go_installed=true
            else
                for _gm in "${_go_mirrors[@]}"; do
                    _start_spinner "Downloading Go compiler..."
                    if curl -fSL --max-time 300 "$_gm" -o /tmp/neonvpn_go.tar.gz 2>/dev/null; then
                        _stop_spinner
                        tar -C /usr/local -xzf /tmp/neonvpn_go.tar.gz 2>/dev/null
                        rm -f /tmp/neonvpn_go.tar.gz
                        export PATH=$PATH:/usr/local/go/bin
                        if command -v go &>/dev/null; then
                            log_ok "Go terinstall: $(go version)"
                            _go_installed=true
                            break
                        fi
                    else
            ...
            if [[ "$_go_installed" != "true" ]]; then
                echo ""
                log_fail "Tidak bisa install Go compiler juga!"
                log_info "VPS kamu memblokir hampir semua server download eksternal."
            fi
        fi
        
        # If Go is available, compile Xray from source
        if command -v go &>/dev/null; then
            log_info "Mengcompile Xray dari source (ini membutuhkan waktu 2-5 menit)..."
            _start_spinner "Compiling Xray from source..."
            export GOPROXY=https://goproxy.cn,https://proxy.golang.org,direct
            export GO111MODULE=on
            if go install github.com/xtls/xray-core@latest 2>/dev/null; then
                _stop_spinner
                # Go installs to $GOPATH/bin or $HOME/go/bin
                _go_xray="$(go env GOPATH 2>/dev/null)/bin/xray"
                [[ ! -f "$_go_xray" ]] && _go_xray="$HOME/go/bin/xray"
                if [[ -f "$_go_xray" ]]; then
                    install -m 755 "$_go_xray" "$XRAY_BIN"
                    log_ok "Xray tercompile dan terinstall: ${BWHT}$($XRAY_BIN version 2>/dev/null | head -1)${RST}"
                    xray_downloaded=true
                else
                    _stop_spinner 2>/dev/null
                    log_fail "Compile berhasil tapi binary tidak ditemukan di $_go_xray"
                fi
            else
                _stop_spinner 2>/dev/null
                log_fail "Compile dari source gagal!"
            fi
        fi
    fi
    ...
    if [[ "$xray_downloaded" != "true" ]]; then
        echo ""
        log_fail "Semua metode download & compile Xray gagal!"
        log_info "VPS kamu memblokir akses ke GitHub, CDN, dan Go proxy."
        echo ""
        echo -e "  ${BWHT}=== SOLUSI MANUAL ===${RST}"
        echo -e "  ${SILVER}1. Download file ini dari HP/komputer (bukan dari VPS):${RST}"
        echo -e "  ${DIM}   https://github.com/XTLS/Xray-core/releases/download/v${XRAY_VER}/${XRAY_FILENAME}${RST}"
        echo -e "  ${DIM}   (atau cari di Google: 'Xray-core releases download')${RST}"
        echo ""
        echo -e "  ${SILVER}2. Upload ke VPS menggunakan WinSCP/FileZilla atau scp:${RST}"
        echo -e "  ${DIM}   scp ${XRAY_FILENAME} root@$(get_server_ip):/tmp/${RST}"
        echo ""
        echo -e "  ${SILVER}3. Ekstrak & install manual di VPS:${RST}"
        ...
        echo ""
        echo -e "  ${SILVER}4. Jalankan ulang installer: ${BWHT}neonvpn${RST}"
        echo -e "  ${DIM}   (installer akan skip download karena xray sudah ada)${RST}"
        echo ""
        exit 1
    fi
    fi  # end else: xray not installed
